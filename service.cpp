...
/*
    if (id_service.isEmpty()) {
        qDebug() << "id_service is empty. Cannot insert.";
        return false;
    }
    ...
    if (!checkQuery.exec()) {
        qDebug() << "Error checking for existing id_service:" << checkQuery.lastError().text();
        return false;
    }

    if (checkQuery.next() && checkQuery.value(0).toInt() > 0) {
        qDebug() << "Service with id_service" << id_service << "already exists.";
        return false;
    }
*/
...
QSqlQueryModel* service::search( QString &searchid) {
    ...
    QString queryStr = QString("SELECT * FROM SERVICE WHERE ID_SERVICE LIKE '%%1%'").arg(searchid);
...
/*
QStringList service::getServiceIds() {
    QStringList serviceIds;

    // Create the query to select all ID_SERVICE values from the SERVICE table
    QSqlQuery query;
    query.prepare("SELECT ID_SERVICE FROM SERVICE");

    if (query.exec()) {
        // Iterate through the result set and add each ID_SERVICE to the list
        while (query.next()) {
            QString id_service = query.value(0).toString();  // Get ID_SERVICE from the first column
            qDebug() << "Retrieved ID_SERVICE:" << id_service;  // Debug log
            serviceIds.append(id_service);  // Append to the list
        }
    } else {
...
QList<task> task::afficher() {
    ...
    if (!query.exec()) {
        qDebug() << "SQL Error:" << query.lastError().text();
        return tasks; // Return empty list if query fails
    }

    // Iterate through the results and create task objects
    while (query.next()) {
        QString id_service = query.value(0).toString(); // Fetch the service ID as QString
        QString nom_client = query.value(1).toString();
        QString modele = query.value(2).toString();
        QString service = query.value(3).toString();
        QDate date_task = query.value(4).toDate();
        QString priority = query.value(5).toString(); // Assuming priority is stored as a QString
        int prog = query.value(6).toInt();

        // Create a task object using the retrieved data
        task t(id_service, nom_client, modele, service, date_task, priority, prog);
        tasks.append(t); // Append each task to the list
    }
...
bool task::updateTask(QString id_task) {
    QSqlQuery query;
    query.prepare("UPDATE TASK SET CLIENT = :client, MODELE = :modele, SERVICE = :service, DATE = :date, "
                  "PRIX = :prix, PROG = :prog WHERE ID_SERVICE = :id_task");
...
QSqlQueryModel* task::afficherTasks() {
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query("SELECT * FROM TASK");

    // Move the query into the model instead of copying it
